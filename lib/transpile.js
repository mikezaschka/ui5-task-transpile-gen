...
module.exports = async function ({
    ...
    const preloadFileName = options.configuration && options.configuration.preloadFileName || "Component.js"
    
    // Get path of Component
    const componentResources = jsResources.filter((jsResource) => jsResource.getPath().includes("Component.js") || jsResource.getPath().includes("library.js"));
    for (const componentResource of componentResources) {
        await handleRuntime(componentResource);
    }

    /**
     * Prepends the runtime to the Component/Library
     * 
     * @param {*} componentResource 
     */
    async function handleRuntime(componentResource, preloadFileName) {
     
        const toPath = componentResource.getPath();
        
        let pathPrefix = toPath.replace("Component.js", "");
        pathPrefix = pathPrefix.replace("library.js", "");

        //get path of regenerator in node_modules
        const pathRegenerator = pathregen.substr(pathregen.indexOf("node_modules/regenerator-runtime") + 13).replace("\\", "/");
        //build full path for regenerator in current project
        const virtualPathRegenerator = pathPrefix + pathRegenerator;

        //console.info("\n" + baseLogTask + "Add regenerator-runtime ".green + pathRegenerator + emoji.get('palm_tree'));
        console.log(pathregen);
        console.log(pathRegenerator);
        ...
        console.info(baseLogTask + `Include regenerator-runtime to ${toPath}`.green + emoji.get('palm_tree'));
